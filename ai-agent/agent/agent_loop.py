            ...
            if isinstance(tool_result, dict):
                ...
                # 2. get_connection_data
                    ...
                    if conn_data:
                        ...
                        if customer_name_query == "this_month_due":
                            ...
                            rule_based_response = f"Total dues for this month: ₹{month_meter.get('dues', '0.00')} (Total invoiced: ₹{month_meter.get('total_amount', '0.00')})"
                        elif customer_name_query == "this_month_collection":
                            month_meter = conn_data.get("collection_this_month_meter", {})
                            rule_based_response = f"Total collection for this month: ₹{month_meter.get('collection', '0.00')} ({month_meter.get('percentage', '0')}% collected)"
                    ...
                    if isinstance(unpaid_data, dict):
                        ...
                        if not customers:
                            ...
                            if any(k in msg_lower for k in ["most balance", "highest dues", "most unpaid", "highest balance", "maximum dues"]):
                                ...
                                rule_based_response = f"The customer with the highest dues/balance is {cname} ({area}) with an unpaid balance of ₹{format_curr(balance)}."
                            else:
                                lines = [
                        ...
                        if customer_name_query == "latest":
                            ...
                            if not customers:
                                ...
                                for c in customers[:5]:
                                    ...
                                    lines.append(f"• {cname} (Sub ID: {sub_id}) — Joined: {jdate}")
                                if total_count > 5:
                                    lines.append("\nFor the remaining, click the link below.")
                                rule_based_response = "\n".join(lines)
                        else:
                            lines = [f"Total customers: {total_count}", "\nHere are the first few customers:"]
                            for c in customers[:5]:
                                cname = c.get("name") or "Unknown"
                                sub_id = c.get("subscriber_id") or "N/A"
                                lines.append(f"• {cname} (Sub ID: {sub_id}) — Status: {c.get('status')}")
                            if total_count > 5:
                                lines.append("\nFor the remaining, click the link below.")
                            rule_based_response = "\n".join(lines)
                ...
                # 15. get_cancelled_invoices
                    ...
                    if not items:
                        ...
                        lines = [f"Total cancelled invoices: {total}", "Here are the recent cancelled invoices:"]
                        for item in items[:5]:
                            ...
                            lines.append(f"• #{inv_no} — {cname} (Sub ID: {sub_id}): ₹{amount} on {date}")
                        if total > 5 or len(items) > 5:
                            lines.append("\nFor the remaining, click the link below.")
                        rule_based_response = "\n".join(lines)
